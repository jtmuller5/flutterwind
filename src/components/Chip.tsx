...
export function Chip({ label, onDelete, color = "bg-gray-200", className = "" }: ChipProps) {
  return (
    <div
      className={`inline-flex items-center ${color} ${className} px-2 py-1 rounded-full text-sm`}
    >
      {label}
      {onDelete && (
